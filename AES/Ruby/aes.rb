require 'debugger'

class AESCipher

  def initialize args
    @cipher_key = args[:cipher_key].split
    @nb = args[:nb]
    @nk = args[:nk]
    @nr = args[:nr]
    @state = Array.new(4){Array.new(@nb){""}}
    
    @rcon = [ 
      "00000000",
      "01000000",
      "02000000",
      "04000000",
      "08000000",
      "10000000",
      "20000000",
      "40000000",
      "80000000",
      "1b000000",
      "36000000"
    ]
    
    @sbox = [  ]
  end

  def cipher input

    key_schedule = key_expansion
    input = input.split
    for r in 0...4
      for c in 0...@nb
        @state[r][c]=input[r+4*c]
      end
    end
    
    add_round_key(key_schedule, 0)

    for round in 1...@nr 
      sub_bytes
      shift_rows
      mix_columns
      add_round_key(key_schedule, round)
    end

    sub_bytes
    shift_rows
    add_round_key(key_schedule, @nr)

    output = []
    for r in 0...4
      for c in 0...@nb
        output[r+4*c]=@state[r][c]
      end
    end

    output.join(" ")
  end

  private

  def key_expansion
    word = []
    for i in 0...@nk
      word << [@cipher_key[4*i], @cipher_key[4*i+1], @cipher_key[4*i+2], @cipher_key[4*i+3]].join
    end

    limit = @nb * (@nr + 1)
    for i in @nk...limit
      temp = word[i-1].clone
      if (i % @nk == 0)
        temp = (sub_word(rot_word(temp)).hex ^ @rcon[i/@nk].hex).to_s(16)
      elsif (@nk > 6 && i % @nk == 4)
        temp = sub_word(temp)
      end
      word[i] = (word[i-@nk].hex ^ temp.hex).to_s(16)
    end
    word
  end

  def sub_word(temp)
    (temp.length/2).times do
      byte = temp.slice!(0,2)
      temp << @sbox[byte.hex].to_s(16).rjust(2, '0')
    end
    temp
  end
  
  def rot_word(temp)
    first = temp.slice!(0,2)
    temp << first
  end
  
  def add_round_key(key_schedule, round)
    for r in 0...4
      for c in 0...@nb
        @state[r][c] = (@state[r][c].hex ^ key_schedule[round*4+c][r*2, 2].hex).to_s(16).rjust(2, '0')
      end
    end
  end

  def sub_bytes
    for r in 0...4
      for c in 0...@nb
        @state[r][c] = @sbox[@state[r][c].hex].to_s(16).rjust(2, '0')
      end
    end
  end

  def shift_rows
    4.times{ |i| @state[i].rotate!(i) }
  end

  def mix_columns
    for c in 0...4
      a = []
      b = []
      for i in 0...4
        a[i] = @state[i][c].hex
        h = @state[i][c].hex >> 7
        b[i] = @state[i][c].hex << 1
        b[i] ^= 0x1b if h > 0
        b[i] %= 256
      end

      @state[0][c] = (b[0] ^ a[1] ^ b[1] ^ a[2] ^ a[3]).to_s(16).rjust(2, '0')
      @state[1][c] = (a[0] ^ b[1] ^ a[2] ^ b[2] ^ a[3]).to_s(16).rjust(2, '0')
      @state[2][c] = (a[0] ^ a[1] ^ b[2] ^ a[3] ^ b[3]).to_s(16).rjust(2, '0')
      @state[3][c] = (a[0] ^ b[0] ^ a[1] ^ a[2] ^ b[3]).to_s(16).rjust(2, '0')
    end
  end
end

crypt = AESCipher.new(cipher_key: "2b 7e 15 16 28 ae d2 a6 ab f7 15 88 09 cf 4f 3c", nk: 4, nr: 10, nb: 4)
puts "input:  32 43 f6 a8 88 5a 30 8d 31 31 98 a2 e0 37 07 34"
puts "key:    2b 7e 15 16 28 ae d2 a6 ab f7 15 88 09 cf 4f 3c"
print "output: "
puts crypt.cipher("32 43 f6 a8 88 5a 30 8d 31 31 98 a2 e0 37 07 34")
